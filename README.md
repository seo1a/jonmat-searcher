...
꽤 오래 전 기획했고 Vanilla JS를 사용해 구현했던 첫 프로젝트였습니다. 그 당시에는 기술적으로 아쉬운 점이 상당히 많은 채로 끝마쳤던 프로젝트였는데, 이번에는 리액트, 자바스크립트 및 여러 다른 기술 스택을 사용해 다시 구현해보았습니다.<br> 


맛집 검색을 위해 여러 플랫폼을 일일이 방문하는 번거로움을 없애고, 3가지 플랫폼의 리뷰를 빠르게 비교할 수 있게 하기 위함을 첫번째 목표로 설정했습니다. <br>
검색창에 맛집 이름을 검색하면 후기와 장소가 표시될 것임을 별도의 설명 없이도 알 수 있도록 구현하고 싶었습니다. 따라서 누구나 쉽게 쓸 수 있도록 직관적인 `싱글 페이지 UI`를 설계했으며, `모바일 최적화`도 함께 고려했습니다.<br>
